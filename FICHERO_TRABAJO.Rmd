...
author: "Pere Caimari Fuster, Enrique Sayas Bailach, Lucas Contreras Vilar, Diego Martin Soler"
date: '`r Sys.Date()`'
output:
  html_document:
    echo: yes
...
```{r}
ggplot(sonido_tidy, aes(x = CALLE, y = SDTN)) + geom_boxplot()
ggplot(sonido_tidy, aes(x = CALLE, y = S7_19)) + geom_boxplot()
ggplot(sonido_tidy, aes(x = CALLE, y = NSC)) + geom_boxplot()
ggplot(sonido_tidy, aes(x = CALLE, y = S19_23)) + geom_boxplot()
ggplot(sonido_tidy, aes(x = CALLE, y = S23_7)) + geom_boxplot()
```

# Creación de los estadisticos 

```{r}
estadisticos<- sonido_tidy%>% filter(CALLE == 'cadiz16') %>% select(NSC:S23_7)  %>% summarise_at(list(min=min, max=max, med=mean , mediana=median,sd=sd))

quantiles <- estadistico %>% group_by(asignaturas)  %>% summarize(quant25 = quantile(notas,probs = q[1],na.rm = T),quant75 = quantile(notas,probs = q[3],na.rm = T))
```
